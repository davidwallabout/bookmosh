...
serve(async (req: Request) => {
  ...
  try {
    ...
    switch (type) {
      ...
      case 'friend_invite':
        subject = `${data.inviterName || 'A friend'} invited you to join BookMosh`
        const inviteUrl = `${appUrl}?invitedBy=${encodeURIComponent(data.inviterName || '')}`
        html = `
          <!DOCTYPE html>
          <html>
            <head>
                ...
                <tr>
                  <td align="center">
                    <table width="600" cellpadding="0" cellspacing="0" style="background-color: #141b2d; border-radius: 16px; overflow: hidden; border: 1px solid rgba(255, 255, 255, 0.1);">
                      ...
                      <tr>
                        <td style="padding: 0 40px 40px;">
                          <h2 style="color: #ffffff; font-size: 24px; margin: 0 0 20px; font-weight: 600;">You're invited to BookMosh!</h2>
                          <p style="color: rgba(255, 255, 255, 0.7); font-size: 16px; line-height: 1.6; margin: 0 0 20px;">
                            <strong style="color: #ffffff;">${data.inviterName || 'A friend'}</strong> invited you to join BookMosh — a cozy place to track what you're reading and share it with friends.
                          </p>
                          <table width="100%" cellpadding="0" cellspacing="0" style="margin-top: 30px;">
                            <tr>
                              <td align="center">
                                <a href="${inviteUrl}" style="display: inline-block; background: linear-gradient(135deg, #7df9ff 0%, rgba(255, 255, 255, 0.7) 100%); color: #0b1225; text-decoration: none; padding: 14px 32px; border-radius: 12px; font-weight: 600; font-size: 14px; text-transform: uppercase; letter-spacing: 0.1em;">
                                  Join BookMosh
                                </a>
                              </td>
                            </tr>
